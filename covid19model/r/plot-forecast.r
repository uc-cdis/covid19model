...
make_two_plots <- function(data_country, data_country_forecast, filename, country, code) {
  ...
  p <- ggplot(data_country) +
    ...
    geom_bar(
      data = data_country, aes(x = time, y = deaths),
      fill = "coral4", stat = "identity", alpha = 0.5
    ) +
    geom_line(
      data = data_country, aes(x = time, y = estimated_deaths),
      col = "deepskyblue4"
    ) +
  ...
  p <- ggplot(data_country) +
    ...
    geom_line(
      data = data_country, aes(x = time, y = predicted_cases),
      col = "deepskyblue4"
    ) +
    # geom_line(data = data_country_forecast,
